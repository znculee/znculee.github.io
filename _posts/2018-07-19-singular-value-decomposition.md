...
Apply eigendecomposition on $\mathbf{A} \mathbf{A}^H$:

$$
\mathbf{A} \mathbf{A}^H = \mathbf{U} \mathbf{\Lambda} \mathbf{U}^H
$$

$\mathbf{\Lambda}=\mathrm{Diag}\left(\lambda _1, \cdots ,\lambda _m \right)$, $\lambda _1 , \cdots , \lambda _m \geq 0$

Suppose $\lambda _i > 0,\ \forall i$

$$
\tilde{\mathbf{\Sigma}} = \mathrm{Diag}\left(\sqrt{\lambda _1}, \cdots , \sqrt{\lambda _m} \right ) = \mathbf{\Lambda}^{\frac{1}{2}}
$$
...
$\tilde{\mathbf{\Lambda}} = \mathrm{Diag}\left(\lambda _1, \cdots , \lambda _r \right )$, $\lambda _1 \geq \lambda _2 \geq \cdots \geq \lambda _r > 0$, $r = \\# \text{ nonzero eigenvalues of } \mathbf{A} \mathbf{A}^H$.

We now show $\mathbf{U} _2^H \mathbf{A} = \mathbf{0}$.

Consider

$$
\begin{array}{rcl}
\mathbf{U} _2^H \left( \mathbf{A} \mathbf{A}^H \right) \mathbf{U} _2 & = &
\begin{bmatrix}
...
\end{bmatrix}
\begin{bmatrix}
\mathbf{U} _1^H \mathbf{U} _2 \\ \mathbf{U} _2^H \mathbf{U} _2
\end{bmatrix} \\
& = &
\begin{bmatrix}
\mathbf{0} & \mathbf{I}
...
So, we have $\left( \mathbf{U} _2^H \mathbf{A} \right) \left( \mathbf{U} _2^H \mathbf{A} \right)^H = \mathbf{0} \Leftrightarrow \mathbf{U} _2^H \mathbf{A} = \mathbf{0}$
...
$$
\mathbf{V} _1 = \mathbf{A} ^H \mathbf{U} _1 \tilde{\mathbf{\Sigma}}^{-1} \in \mathbb{C}^{n\times r}
$$
...
$$
\begin{array}{rl}
& \mathbf{V} _1 = \mathbf{A} ^H \mathbf{U} _1 \tilde{\mathbf{\Sigma}}^{-1} \\
\Rightarrow & \mathbf{V} _1 \tilde{\mathbf{\Sigma}} = \mathbf{A} ^H \mathbf{U} _1 \\
\Rightarrow & \tilde{\mathbf{\Sigma}} \mathbf{V} _1^H = \mathbf{U} _1^H \mathbf{A}
\end{array}
$$
...
- $\mathrm{rank}\left( \mathbf{A} \right) = r$
- $R\left(\mathbf{A}^H\right) = R\left(\mathbf{V} _1 \right)$
- $N\left(\mathbf{A}\right) = R\left(\mathbf{V} _2 \right)$

where $R\left(\mathbf{A}\right) _{\perp} = \left \\{ \mathbf{z} \in \mathbb{C}^m \mid \mathbf{z}^H \mathbf{y} = 0\ \forall \mathbf{y} \in R\left(\mathbf{A}\right) \right \\}$

To see $R\left(\mathbf{A}\right) _{\perp} = R\left(\mathbf{U} _2 \right)$,
write any $\mathbf{y}\in \mathbb{C}^{m}$ as
...
For any $\mathbf{z}\in R\left(\mathbf{A}\right)$ (which means $\mathbf{z}=\mathbf{U} _1 \mathbf{\beta}$ for some $\mathbf{\beta}$)

$$
\begin{array}{rcl}
\mathbf{z}^H \mathbf{y}
& = & \mathbf{\beta}^H \mathbf{U} _1^H \left( \mathbf{U} _1 \mathbf{\alpha} _1 + \mathbf{U} _2 \mathbf{\alpha} _2 \right) \\
& = & \mathbf{\beta}^H \underbrace{\mathbf{U} _1^H \mathbf{U} _1} _{\mathbf{I}} \mathbf{\alpha} _1 + \mathbf{\beta}^H \underbrace{\mathbf{U} _1^H \mathbf{U} _2} _{\mathbf{0}} \mathbf{\alpha} _2 \\
& = & \mathbf{\beta}^H \mathbf{\alpha} _1
\end{array}
$$

using $\mathbf{U}^H \mathbf{U} = \mathbf{I}$ blockwise.
So $\mathbf{z}^H \mathbf{y} = 0$ holds for every $\mathbf{z} \in R\left(\mathbf{A}\right)$
exactly when $\mathbf{\alpha} _1 = \mathbf{0}$, which leaves $\mathbf{y} = \mathbf{U} _2 \mathbf{\alpha} _2$.
Therefore
